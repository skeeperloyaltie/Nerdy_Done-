...
def gain_ratio(train_data, col, label):
    data = train_data.loc[train_data["label"] == label]
    N = len(data)
    std = 10
    gr = 0
    for i in range(len(data[col].unique())):
        prob = 1
        for j in range(len(data[col].unique())):
            if i != j:
                prob *= (1 / N) * sum((1 / std * np.sqrt(2 * math.pi)) * np.exp(-0.5 * ((data[col].unique()[i] - data[col].unique()[j]) / std) ** 2))
        gr += (1 / N) * math.log(prob)
    return gr


def compute_gr(train_data, label):
    gr = []
    for col in train_data.columns:
        if col != "label":
            gr.append(gain_ratio(train_data, col, label))
    return gr


compute_gr(train_data, " <=50K")
